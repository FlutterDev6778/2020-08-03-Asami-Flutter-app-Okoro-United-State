...
class AuthProvider extends ChangeNotifier {
  ...
  Future<void> loginWithEmailOnFirebase({@required String email, @required String password, bool remeberMe = true, bool isAdmin = false}) async {
    /// firebase authentication with email and password
    ...
    Map<String, dynamic> userResult = await UserDataProvider.getUserByUID(authResult["user"].uid, isAdmin: isAdmin);
    if (!userResult["state"]) {
      _errorString = userResult["errorString"];
      _processingState = -1;
      notifyListeners();
